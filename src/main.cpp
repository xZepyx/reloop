...
int main(int argc, char* argv[])
{
    ...
    std::string watchFile = "etc/mtab"; // Default file to watch
    std::string command = "echo 'File changed!'"; // Default command to execute on file change
...
*/
    ...
    for (int i = 1; i < argc; ++i) { // Start from i=1 to skip program name
        ...
        if (arg == "--command" && (i + 1) < argc) { // Command that is executed on file change (in "")
    ...
    filewatch::FileWatch<std::string> watch(
        ...
        [&command, &last_run](const std::string& path, const filewatch::Event change_type) {
            if (change_type == filewatch::Event::modified) {
                ...
                if (diff > 500) { // only run if more than 500ms passed
...
// I'm making to much comments, LOL.